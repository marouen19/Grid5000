...
begin
  Restfully::Session.new(:base_uri => config['url']+'/sid/grid5000', :logger => logger, :username => config['username'], :password => config['password']) do |root, session|
    root.sites.each do |site|
      ...
      if free_nodes > 1
        jobs << site.jobs.submit(:resources => "nodes=BEST,walltime=00:20:00", :command => "sleep 1800", :types => ["deploy"], :name => "API Main Practical") rescue nil
      else
    ...
    begin
      Timeout.timeout(60*15) do # wait at most 15 mins
    ...
    deployments.each do |deployment|
      if deployment['status'] != 'terminated'
        deployment.delete
	      ...
	      print "all environements are deployed"
